...
from solden.core.money import Q2, money_to_float, to_decimal
from solden.integrations.erp_sanitization import _sanitize_odata_value
...
async def _post_bill_to_sap_b1(
    ...
    cockpit views can filter on Solden-managed work.
    """
    ...
    # Tax-code propagation: Solden vat_code -> SAP B1 TaxCode.
...
async def _post_to_sap_s4hana(
    ...
    payload = {
        ...
        "DocumentReferenceID": (
            str(entry.get("reference_id") or "Solden")[:16]
        ),
...
async def _post_bill_to_sap_s4hana(
    ...
    # Map Solden vat_code to S/4HANA MWSKZ. Per-tenant overrides
...
# Default Solden vat_code -> S/4HANA MWSKZ tax code map.
