...
from solden.core.org_utils import coerce_org_id
...
def _active_org_ids() -> List[str]:
    ...
    try:
        from solden.services.email_tasks import get_tasks
...
def _collect_org_overdue_and_stale_tasks(
    ...
    *,
    ...
    from solden.services.email_tasks import get_overdue_tasks, get_tasks
...
async def start_agent_background(app=None):
    ...
    from solden.core.observability import capture_background_exception
    ...
    while True:
        try:
            ...
            if tick % 4 == 0:
                for org_id in org_ids:
                    try:
                        from solden.services.circuit_breaker import check_circuit_breaker
...
async def _check_overdue_tasks():
    ...
    try:
        try:
            from solden.services.task_scheduler import log_reminder, should_send_reminder
        except Exception:
...
async def _check_anomalies(org_id: str):
    ...
    try:
        from solden.services.agent_anomaly_detection import AnomalyDetectionService
...
async def _check_approval_timeouts(org_id: str):
    ...
    try:
        ...
        from solden.services.slack_notifications import send_approval_reminder
...
async def _poll_payment_statuses(org_id: str) -> Dict[str, int]:
    ...
    try:
        from solden.core.database import get_db
        from solden.integrations.erp_router import get_bill_payment_status
...
async def _sweep_exception_resolutions(org_id: str):
    ...
    try:
        from solden.core.database import get_db
        from solden.services.exception_resolver import get_exception_resolver
...
async def _check_period_end(org_id: str):
    ...
    try:
        from solden.services.agent_monitoring import detect_period_end
...
async def _sync_purchase_orders(org_id: str):
    ...
    """
    try:
        from solden.integrations.erp_router import sync_purchase_orders_from_erp
...
async def _deliver_scheduled_reports(org_id: str):
    ...
    try:
        from solden.services.scheduled_reports import get_scheduled_report_service
...
async def _run_monitoring_checks(org_id: str):
    ...
    try:
        from solden.services.monitoring import run_monitoring_checks
...
async def _check_queue_depth_and_concurrency() -> Dict[str, Any]:
    ...
    # 1. Queue depth check
    try:
        from solden.core.event_queue import (
    ...
    try:
        from solden.services.workspace_semaphore import (
            WorkspaceSemaphore,
        )
        from solden.core.database import get_db
        db = get_db()
