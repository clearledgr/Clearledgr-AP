...
with a payment hold (see :mod:`solden.services.erp_webhook_dispatch`),
...
from solden.core.ap_states import APState, validate_transition
...
async def _handle_approve(
    ...
    if source == "netsuite_native":
        ...
        from solden.integrations.erp_sap_s4hana import release_payment_block as release_sap_block
...
async def _handle_reject(
    ...
    if source == "netsuite_native":
        ...
        from solden.integrations.erp_sap_s4hana import cancel_supplier_invoice as cancel_sap_invoice
        cc = str(metadata.get("sap_company_code") or "").strip()
        doc = str(metadata.get("sap_supplier_invoice") or "").strip()
        fy = str(metadata.get("sap_fiscal_year") or "").strip()
        if not (cc and doc and fy):
