...
from solden.api.gmail_extension_models import (  # noqa: F401 — re-exported for back-compat
    EmailTriageRequest,
    EmailProcessRequest,
    BulkScanRequest,
...
from solden.api.gmail_extension_common import (
...
def _gmail_extension_support_module():
    ...
    if _GMAIL_EXTENSION_SUPPORT is None:
        from solden.services import gmail_extension_support as module
...
def _gmail_api_module():
    ...
    if _GMAIL_API_MODULE is None:
        from solden.services import gmail_api as module
...
def _extract_field_confidences(*args, **kwargs):
    from solden.core.ap_confidence import extract_field_confidences
...
def get_audit_service():
    from solden.api.deps import get_audit_service as _get_audit_service

    return _get_audit_service()


def get_audit_trail():
    from solden.services.audit_trail import get_audit_trail as _get_audit_trail

    return _get_audit_trail()


def get_vendor_intelligence():
    from solden.services.vendor_intelligence import get_vendor_intelligence as _get_vendor_intelligence

    return _get_vendor_intelligence()


def get_policy_compliance(organization_id: str):
    from solden.services.policy_compliance import get_policy_compliance as _get_policy_compliance

    return _get_policy_compliance(organization_id)


def get_priority_detection(organization_id: str):
    from solden.services.priority_detection import get_priority_detection as _get_priority_detection

    return _get_priority_detection(organization_id)


def get_budget_awareness(organization_id: str):
    from solden.services.budget_awareness import get_budget_awareness as _get_budget_awareness

    return _get_budget_awareness(organization_id)


def build_worklist_item(*args, **kwargs):
    from solden.services.ap_item_service import build_worklist_item as _build_worklist_item

    return _build_worklist_item(*args, **kwargs)


def build_worklist_items(*args, **kwargs):
    from solden.services.ap_projection import build_worklist_items as _build_worklist_items
    ...
    *,
    ...
    for message in messages:
        ...
        if not finance_email:
            try:
                from solden.api.gmail_webhooks import process_single_email
...
async def triage_email(
    ...
    from solden.services.gmail_triage_service import (
...
async def repair_historical_invoices(
    ...
    from solden.api.gmail_webhooks import process_invoice_email
...
async def cleanup_gmail_labels(
    ...
    from solden.services.gmail_labels import cleanup_legacy_labels
...
async def register_gmail_token(request: RegisterGmailTokenRequest):
    ...
    if user is None:
        ...
        from solden.core.auth import create_user_from_google
...
async def exchange_gmail_code(request: ExchangeCodeRequest):
    ...
    """
    from solden.services.gmail_api import exchange_code_for_tokens
    from solden.core.auth import create_user_from_google
...
async def match_bank_feed(
    ...
    """
    org_id = _resolve_org_id_for_user(_user, request.organization_id)
    from solden.workflows.gmail_activities import match_bank_feed_activity
...
async def match_erp(
    ...
    """
    org_id = _resolve_org_id_for_user(_user, request.organization_id)
    from solden.workflows.gmail_activities import match_erp_activity
...
def _load_idempotent_extension_response(db: Any, idempotency_key: Optional[str]) -> Optional[Dict[str, Any]]:
    """Thin wrapper over :func:`solden.core.idempotency.load_idempotent_response`.
