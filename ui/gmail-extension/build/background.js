...
function runBrowserCommand(command) {
  ...
  if (tool === 'find_element') {
    ...
    if (!element) return { ok: false, error: 'not_found', selector };
    return {
      ok: true,
      selector,
  ...
  if (tool === 'type') {
    ...
    if ('value' in element) {
      ...
      return { ok: true, selector, value_length: value.length };
    }
    return { ok: false, error: 'element_not_input', selector };
  }
  ...
  if (tool === 'select') {
    ...
    return { ok: false, error: 'element_not_select', selector };
  }
