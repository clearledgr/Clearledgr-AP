...
class FinanceAgentRuntime:
    ...
    def _evaluate_action_autonomy_policy(
        ...
        *,
        ...
        if status != "ready" or failing_gates:
            ...
            recent_invoice_count = self._safe_int(scorecard.get("recent_invoice_count"))
            sample_recommended_count = self._safe_int(scorecard.get("sample_recommended_count"))
            source_shift_fields = [
                str(value).strip()
                for value in (scorecard.get("source_shift_fields") or [])
                if str(value).strip()
            ]
            verification_attempted_count = self._safe_int((verification_scorecard or {}).get("attempted_count"))
            verification_rate = self._safe_float((verification_scorecard or {}).get("verification_rate"))
            verification_mismatch_count = self._safe_int((verification_scorecard or {}).get("mismatch_count"))
            ...
            if recent_invoice_count < self._safe_int(thresholds.get("min_recent_invoice_count")):
            ...
            min_shadow_scored = self._safe_int(thresholds.get("min_shadow_scored_item_count"))
            if min_shadow_scored > 0:
                shadow_scored_count = self._safe_int((shadow_scorecard or {}).get("scored_item_count"))
                if not shadow_scorecard or shadow_scored_count < min_shadow_scored:
                    reason_codes.append("vendor_shadow_observation_mode")
                else:
                    shadow_action_match_rate = self._safe_float((shadow_scorecard or {}).get("action_match_rate"))
                    shadow_critical_field_match_rate = self._safe_float((shadow_scorecard or {}).get("critical_field_match_rate"))
                    shadow_disagreement_count = self._safe_int((shadow_scorecard or {}).get("disagreement_count"))
                    if shadow_action_match_rate < self._safe_float(thresholds.get("min_shadow_action_match_rate")):
                        reason_codes.append("vendor_shadow_action_match_low")
                    if shadow_critical_field_match_rate < self._safe_float(
            ...
            min_verification_attempts = self._safe_int(thresholds.get("min_post_verification_attempt_count"))
            if min_verification_attempts > 0:
                attempted_count = self._safe_int((verification_scorecard or {}).get("attempted_count"))
                if not verification_scorecard or attempted_count < min_verification_attempts:
                    ...
                    verification_rate = self._safe_float((verification_scorecard or {}).get("verification_rate"))
                    mismatch_count = self._safe_int((verification_scorecard or {}).get("mismatch_count"))
                    if verification_rate < self._safe_float(thresholds.get("min_post_verification_rate")):
                        reason_codes.append("vendor_post_verification_low")
        ...
        blocked_reason_codes = self._dedupe_reason_codes(reason_codes)
    ...
    def _evaluate_ap_vendor_autonomy(
        ...
        *,
        ...
        for action in _AUTONOMY_ACTION_THRESHOLDS:
            action_policies[action] = self._evaluate_action_autonomy_policy(
                action=action,
                vendor=vendor,
                readiness=readiness,
        ...
        return {
            ...
            "hard_manual_reasons": self._dedupe_reason_codes(hard_manual_reasons),
            "failing_gates": failing_gates,
    ...
    def ap_autonomy_policy(
        ...
        *,
        ...
        vendor = self._normalize_vendor_name(vendor_name)
        evaluation = self._evaluate_ap_vendor_autonomy(
            vendor_name=vendor,
            readiness=readiness,
            ap_kpis=ap_kpis,
        )
        requested_action = str(action or "").strip().lower() or "route_low_risk_for_approval"
        dependencies = self._autonomy_requested_action_dependencies(requested_action)
        dependency_policies = [
        ...
        return {
            ...
            "vendor_shadow_action_match_rate": round(self._safe_float(shadow_scorecard.get("action_match_rate")), 4),
            "vendor_shadow_critical_field_match_rate": round(self._safe_float(shadow_scorecard.get("critical_field_match_rate")), 4),
            "vendor_shadow_disagreement_count": int(shadow_scorecard.get("disagreement_count") or 0),
            "vendor_post_verification_rate": round(self._safe_float(verification_scorecard.get("verification_rate")), 4),
    ...
    def ap_autonomy_summary(self, *, window_hours: int = 168) -> Dict[str, Any]:
        ...
        for payload in (
            ...
            for row in payload:
                if not isinstance(row, dict):
                    continue
                vendor = self._normalize_vendor_name(row.get("vendor_name"))
            ...
            vendor_promotion_status.append(
                {
                    ...
                    "shadow_action_match_rate": round(self._safe_float(shadow_scorecard.get("action_match_rate")), 4),
                    "shadow_critical_field_match_rate": round(self._safe_float(shadow_scorecard.get("critical_field_match_rate")), 4),
                    ...
                    "post_verification_rate": round(self._safe_float(verification_scorecard.get("verification_rate")), 4),
