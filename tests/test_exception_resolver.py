...
# ---------------------------------------------------------------------------
# APSkill tool handler: _handle_resolve_exception
# ---------------------------------------------------------------------------


class TestAPSkillToolHandler:
    def test_handler_with_explicit_exception_code(self):
        from clearledgr.core.skills.ap_skill import _handle_resolve_exception

        mock_resolver = MagicMock()
        mock_resolver.resolve = AsyncMock(
            return_value={"resolved": True, "action": "po_auto_attached", "po_number": "PO-1"}
        )

        mock_db = MagicMock()
        mock_db.get_ap_item.return_value = _make_ap_item()
    ...
    def test_handler_reads_code_from_ap_item(self):
        from clearledgr.core.skills.ap_skill import _handle_resolve_exception

        mock_resolver = MagicMock()
        mock_resolver.resolve = AsyncMock(
            return_value={"resolved": False, "reason": "no_matching_po_in_erp", "suggestion": "x"}
        )
    ...
    def test_handler_no_exception_code(self):
        from clearledgr.core.skills.ap_skill import _handle_resolve_exception
        ...
        assert result["ok"] is True
        assert result["resolved"] is False
        assert result["reason"] == "no_exception_code"

    def test_handler_never_raises(self):
        from clearledgr.core.skills.ap_skill import _handle_resolve_exception

        # Even when get_db raises, the handler should return ok=False
        with patch(
            _PATCH_GET_DB,
...
# ---------------------------------------------------------------------------
# APSkill registration
# ---------------------------------------------------------------------------


class TestAPSkillRegistration:
    def test_resolve_exception_tool_is_registered(self):
        from clearledgr.core.skills.ap_skill import APSkill

        skill = APSkill("org-1")
        tools = skill.get_tools()
        tool_names = [t.name for t in tools]
        assert "resolve_exception" in tool_names
    ...
    def test_tool_count_is_eight(self):
        ...
        skill = APSkill("org-1")
        tools = skill.get_tools()
        # Was 7, now 8 with resolve_exception
        assert len(tools) >= 8

    def test_system_prompt_mentions_resolve_exception(self):
        from clearledgr.core.skills.ap_skill import APSkill
        ...
        assert "resolve_exception" in prompt
